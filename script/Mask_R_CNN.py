...
def get_names(path):
    ...
    color_list.clear()
    color_list = [(125,255,0),(0,255,255),(0,255,0)]
...
def draw_output():
    ...
    for i in range(num_obj):
        ...
        if score >= threshold:
            ...
            str_contents = str(round(score*100))+"%"

            t_size = cv2.getTextSize(str_contents, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
            rectangle_size = t_size[1]
            
            if not len(output_tracking) == 0:
                if not t_id == 0:
                    tr_contents = "ID:"+str(t_id)
                    cv2.rectangle(img, (x1-1,y1-60), (x2+1,y1), (255,255,255), -1)
                    cv2.putText(img,tr_contents,(x1+1, y1-33),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),2)
            
            cv2.rectangle(img, (x1-1,y1-30), (x2+1,y1), (255,255,255), -1)
            cv2.rectangle(img, (x1+5,y1-25), (x1+rectangle_size,y1-25+rectangle_size), color, -1)
            cv2.rectangle(img,(x1,y1),(x2,y2),(255,255,255),2)
            cv2.putText(img,str_contents,(x1+rectangle_size+2, y1-3),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),2)
    '''
